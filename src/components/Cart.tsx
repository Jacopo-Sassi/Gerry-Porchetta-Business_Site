...
export default function Cart({ isOpen, onClose, items, onUpdateCart }: CartProps) {
  ...
  const handleSubmitOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Errore durante l\'invio dell\'ordine. Riprova o contattaci direttamente.');
    } finally {
      setSubmitting(false);
    }
  };
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {orderSuccess ? (
              <div className="text-center py-12">
                ...
                <h3 className="text-2xl font-bold text-stone-800 mb-2">Ordine Ricevuto!</h3>
                <p className="text-stone-600">Ti contatteremo presto per confermare.</p>
              </div>
            ) : isCheckout ? (
                  ...
                  <div className="flex gap-4">
                    <label className="flex-1 cursor-pointer">
                      <input
                        ...
                        onChange={(e) => setFormData({ ...formData, delivery_method: e.target.value as 'pickup' })}
                        className="sr-only"
                      />
                    ...
                    <label className="flex-1 cursor-pointer">
                      <input
                        ...
                        onChange={(e) => setFormData({ ...formData, delivery_method: e.target.value as 'delivery' })}
                        className="sr-only"
                      />
                      <div className={`p-4 border-2 rounded-lg text-center transition-all ${
              ...
              <>
                {items.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-xl text-stone-600">Il tuo carrello è vuoto</p>
                    <p className="text-stone-500 mt-2">Aggiungi prodotti dal menu!</p>
                  </div>
                ) : (
          ...
          {!isCheckout && !orderSuccess && items.length > 0 && (
            <div className="p-6 border-t border-stone-200 bg-stone-50">
              <div className="flex justify-between items-center mb-4">
                <span className="text-lg font-semibold text-stone-700">Totale</span>
